...
lineGeometry.vertices.push(new THREE.Vector3(-0.5, -0.5, 0), new THREE.Vector3(0.5, -0.5, 0), new THREE.Vector3(0.5, 0.5, 0), new THREE.Vector3(-0.5, 0.5, 0), new THREE.Vector3(-0.5, -0.5, 0));
...
export class OrientedImageLoader {
    ...
    static async loadImageParams(path, parsedMetadata) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            let min_x = parsedMetadata.boundingBox.min[0];
            let min_y = parsedMetadata.boundingBox.min[1];
            let min_z = parsedMetadata.boundingBox.min[2];
            let max_x = parsedMetadata.boundingBox.max[0];
            let max_y = parsedMetadata.boundingBox.max[1];
            let max_z = parsedMetadata.boundingBox.max[2];

            if (params.x >= min_x && params.x <= max_x) {
                if (params.y >= min_y && params.y <= max_y) {
                    if (params.z >= min_z && params.z <= max_z) {
    ...
    static async load(cameraParamsPath, imageParamsPath, parsedMetadata, viewer) {
        const tStart = performance.now();

        const [cameraParams, imageParams] = await Promise.all([OrientedImageLoader.loadCameraParams(cameraParamsPath), OrientedImageLoader.loadImageParams(imageParamsPath, parsedMetadata)]);
        ...
        const onMouseMove = (evt) => {
            ...
            if (shouldAddClipVolume || selectionChanged) {
                ...
                camera.rotation.copy(img.mesh.rotation);
                {
                    ...
                    const newCamTarget = new THREE.Vector3().addVectors(newCamPos, newCamDir.clone().multiplyScalar(viewer.getMoveSpeed()));
