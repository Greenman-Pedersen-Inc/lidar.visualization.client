...
export class Images360 extends EventDispatcher {
    ...
    focus(image360) {
        ...
        let dir = new THREE.Vector3(...image360.nextImage360.position).sub(viewer.scene.view.position).normalize();
    ...
    load(image360) {
        return new Promise((resolve) => {
            ...
            // // Set the center of rotation (default is 0.5, 0.5, which is the center of the texture)
...
export class Images360Loader {
                ...
                if (long >= min_x && long <= max_x) {
                    if (lat >= min_y && lat <= max_y) {
                        if (alt >= min_z && alt <= max_z) {
                            ...
                            let image360 = new Image360(file, time, long, lat, alt, bearing, pitch, roll);
    ...
    static createSceneNodes(images360, transform) {
        function calculateRotation(image360) {
            if (image360.nextImage360) {
                // to find the direction, subtract the initial position, from the final position.
                const dir = new THREE.Vector3(
                    image360.nextImage360.longitude - image360.longitude,
                    image360.nextImage360.latitude - image360.latitude,
                    image360.nextImage360.altitude - image360.altitude
                );

                // console.log(dir, course, pitch, roll);
                //normalize the direction vector (convert to vector of length 1)
                // dir.normalize();

                // const origin = new THREE.Vector3(image360.longitude, image360.latitude, image360.altitude);
                // const length = 10;
                // const hex = 0xffff00;

                // const arrowHelper = new THREE.ArrowHelper(dir, origin, length, hex);
                // viewer.scene.scene.add(arrowHelper);
        ...
        for (let i = 0; i < images360.images.length - 1; i++) {
            let image360 = images360.images[i];
            let nextImage360 = images360.images[i + 1];

            if (nextImage360) {
                image360.nextImage360 = nextImage360;
            }
        }
        ...
        for (let image360 of images360.images) {
            ...
            // let imageRotation = calculateRotation(image360);
            ...
            const arrowHelper = new THREE.ArrowHelper(direction, mesh.position, 1, 0xff0000);

            // Add the arrow helper to the scene
            viewer.scene.scene.add(arrowHelper);
            // mesh.rotateOnAxis(new THREE.Vector3(0, 1, 0), 180);
            // mesh.rotation.setFromVector3(imageRotation);
