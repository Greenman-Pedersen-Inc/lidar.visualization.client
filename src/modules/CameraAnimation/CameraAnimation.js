...
export class CameraAnimation extends EventDispatcher {
    ...
    onUpdate(delta) {
        ...
        if (!animation.tStart) {
            animation.tStart = performance.now();
        }

        let tNow = performance.now();
        let elapsed = (tNow - animation.tStart) / 1000;
        let t = elapsed / animation.duration;

        animation.set(t);

        const frame = animation.at(t);

        animation.viewer.scene.view.position.copy(frame.position);
        animation.viewer.scene.view.lookAt(frame.target);

        if (t > 1) {
            animation.setVisible(animation.originalyVisible);

            animation.viewer.removeEventListener('update', animation.onUpdate);
        }
    }
    play() {
