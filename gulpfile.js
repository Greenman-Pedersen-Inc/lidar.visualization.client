...
let workers = {
    ...
    EptZstandardDecoderWorker: ['src/workers/EptZstandardDecoder_preamble.js', 'libs/zstd-codec/bundle.js', 'libs/ept/ParseBuffer.js', 'src/workers/EptZstandardDecoderWorker.js'],
};
...
gulp.task('workers', async function (done) {
    ...
    gulp.src('./libs/copc/laz-perf.wasm').pipe(gulp.dest('./build/potree/workers'));
...
gulp.task(
    'build',
    gulp.series(gulp.parallel('workers', 'lazylibs', 'shaders', 'icons_viewer', 'examples_page'), async function (done) {
...
gulp.task(
    ...
    gulp.parallel('build', 'pack', 'webserver', async function () {
        let watchlist = ['src/**/*.js', 'src/**/**/*.js', 'src/**/*.css', 'src/**/*.html', 'src/**/*.vs', 'src/**/*.fs', 'resources/**/*', 'examples//**/*.json', '!resources/icons/index.html'];
