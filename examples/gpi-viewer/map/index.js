...
require([
...
], (FeatureLayer, VectorTileLayer, Map, MapView, reactiveUtils, Compass, Legend, ScaleBar, BasemapToggle, Field, FeatureTable, projection, SpatialReference) => {
    let animation;
    ...
    const featureServerURL = 'https://services1.arcgis.com/VLhaRwzp3uCQMr7y/ArcGIS/rest/services/njx_2300678_vectorTile/FeatureServer/';

    // const playButton = document.getElementById('play-button');
    // const pauseButton = document.getElementById('pause-button');
    const closeLidarViewerButton = document.getElementById('close-lidar-viewer-button');
    const restartTourButton = document.getElementById('restart-tour-button');
    ...
    function loadMenu(credentials) {
        if (credentials && credentials.token) {
            const expiryTime = credentials.expiry - Date.now();
            if (expiryTime >= 0) {
                setTimeout(() => {
    ...
    loadMenu(credentials)
        .then((flatLidarList) => {
            ...
            function positionCamera(metadata, coordinates) {
                ...
                if (coordinates) {
                    ...
                    centroidZ = view.extent.width / Math.tan((60 * 3.1415926) / 180);
                } else {
        ...
        `);
            ...
            view.when(() => {
                ...
                const stateOutline = new FeatureLayer({
                    url: 'https://services2.arcgis.com/XVOqAjTOJ5P6ngMu/arcgis/rest/services/NJ_State_Boundary/FeatureServer/0',
                    renderer: {
                        type: 'simple',
                        symbol: {
                            type: 'simple-fill',
                ...
                tour.addStep({
                    id: 'step-1',
                    text: 'Welcome to the GPI Interactive Lidar Viewer! This tour will get you acquainted with the functionality of the application and help you get started with exploring the data available.',
                    buttons: [
                        {
                            text: 'Disable Tour',
                            action: () => {
                ...
                tour.addStep({
                    ...
                    text: 'The asset filter allows you to build queries using any attributes that have been collected in the extraction process. Once the row has been created, another will be created below it to allow you to combine as many as need to create your selection.',
                    attachTo: {
                        element: '.asset-filter-toggle:nth-of-type(2)',
                        on: 'left',
                    },
                    buttons: [
                        {
                ...
                tour.addStep({
                    id: 'step-3',
                    floatingUIOptions: { middleware: [offset(12)] },
                    text: 'The zoom controls allow you to zoom in and out on the map.',
                    attachTo: {
                        element: '.esri-component.esri-zoom.esri-widget',
                        on: 'right',
                    },
                    buttons: [
                        {
                ...
                tour.addStep({
                    id: 'step-4',
                    floatingUIOptions: { middleware: [offset(12)] },
                    text: 'The scale shows the current scale of the map for reference.',
                    attachTo: {
                        element: '.esri-component.esri-scale-bar.esri-widget',
                        on: 'top',
                    },
                    buttons: [
                        {
                ...
                tour.addStep({
                    id: 'step-5',
                    floatingUIOptions: { middleware: [offset(12)] },
                    text: 'The legend shows the currently visible layers on the map and a brief explanation of what they are.',
                    attachTo: {
                        element: '.esri-component.esri-legend.esri-widget.esri-widget--panel',
                        on: 'right',
                    },
                    buttons: [
                        {
                ...
                tour.addStep({
                    id: 'step-6',
                    floatingUIOptions: { middleware: [offset(12)] },
                    text: 'The feature tables show the assets that are currently selected with the asset filter. They will automatically update when the filter changes. It is possible to select assets by clicking one or many rows. You can then use the context menu on the right to interact with the selection (e.g. zoom to, export, etc.)',
                    attachTo: {
                        element: '.feature-tab-container',
                        on: 'top',
                    },
                    buttons: [
                        {
                ...
                tour.addStep({
                    id: 'step-7',
                    floatingUIOptions: { middleware: [offset(12)] },
                    text: 'The basemap toggle allows the user to switch between the topographic style and hybrid satellite imagery.',
                    attachTo: {
                        element: '.esri-component.esri-basemap-toggle.esri-widget',
                        on: 'right',
                    },
                    buttons: [
                        {
                ...
                tour.addStep({
                    id: 'step-8',
                    floatingUIOptions: { middleware: [offset(12)] },
                    text: 'The "Open Lidar" button appears where the user can view a lidar scan that is associated with the feature selected. Click the "Open Lidar" button to proceed.',
                    beforeShowPromise: function () {
                        return new Promise((resolve) => {
                            rockSlopeFeatureLayer
                                .queryFeatures({
                                    objectIds: [435],
                                    returnGeometry: true,
                                    outFields: ['*'],
                    ...
                    highlightClass: 'extra-highlighted',
                    attachTo: {
                        element:
                            '#map-container .esri-view-root .esri-component.esri-popup.esri-popup--aligned-top-center.esri-popup--shadow .esri-popup__main-container.esri-widget .esri-features.esri-widget.esri-widget--panel calcite-action-bar calcite-action-group calcite-action',
                        on: 'left',
                    },
                    advanceOn: {
                        selector:
                ...
                tour.addStep({
                    id: 'step-9',
                    text: 'This is the lidar data associated with the feature selected from the map. There are various tools here that allow you to interact with the data.',
                    floatingUIOptions: { middleware: [offset(12)] },
                    buttons: [
                        {
                            text: 'Back',
                ...
                tour.addStep({
                    id: 'step-10',
                    text: 'This is the measurement menu. The tools allow you to measure angles, distances, areas, and create profiles.',
                    floatingUIOptions: { middleware: [offset(12)] },
                    attachTo: {
                        element: '#measurement-menu',
                        on: 'top',
                    },
                    buttons: [
                        {
                ...
                tour.addStep({
                    id: 'step-11',
                    text: 'After drawing a profile, click the eyeball to open a seperate window that shows the profile selected.',
                    floatingUIOptions: { middleware: [offset(12)] },
                    attachTo: {
                        element: '[title="View Selected Profile"]',
                        on: 'top',
                    },
                    buttons: [
                        {
                ...
                tour.addStep({
                    id: 'step-11',
                    text: 'The classification tool allows the user to toggle selected classes to better view pertinent data.',
                    floatingUIOptions: { middleware: [offset(12)] },
                    attachTo: {
                ...
                tour.addStep({
                    id: 'step-10',
                    text: 'End of tour, thanks for taking the time to explore the Interactive Lidar Viewer!',
                    buttons: [
                        {
                            text: 'Back',
                            action: () => {
                ...
                restartTourButton.addEventListener('click', (event) => {
                    localStorage.removeItem('seen_tour');
                    tour.start();
                });
            });
        })
